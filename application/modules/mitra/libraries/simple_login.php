...
class simple_login
{
    ...
    * @param string password dari input form*/
    public function login($username, $password)
    {
        ...
        $query = $this->CI->db->get_where('mitra', array('username' => $username, 'password' =>
        ($password)));
        if ($query->num_rows() == 1) {
            //ambil data user berdasar username
            $row = $this->CI->db->query('SELECT * FROM mitra where username = "' . $username . '"');
            $admin = $row->row();
            $id = $admin->id_user;
            $jenis = $admin->jenis;
            ...
            redirect(site_url('mitra/dashboard'));
